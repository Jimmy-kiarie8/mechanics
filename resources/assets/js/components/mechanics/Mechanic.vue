...
<AddMechanic @closeRequest="close" :openAddRequest="dispAdd" @alertRequest="showAlert" :Showrole="AllRoles"></AddMechanic>
<EditMechanic @closeRequest="close" :openEditRequest="pdialog2" :editedItemCon="editedItem"></EditMechanic>
<Profile @closeRequest="close" :openEditRequest="proDialog" :ProfileDetail="routerItem" :doneJobs="JobsDone" :users="Allusers"></Profile>
</div>
</template>

<script>
let AddMechanic = require('./AddMechanic.vue')
let EditMechanic = require('./EditMechanic.vue')
let Profile = require('./Profile.vue')
export default {
  props: ['user', 'role'],
  components: {
    AddMechanic, EditMechanic, Profile
  }, 
  data() {
    return{
      rating: 4,
...
methods: {  
  ...
  editItem(item) {
    ...
    this.editedIndex = this.AllMechanics.indexOf(item)
  ...
  deleteItem(item) {
    ...
    this.editedIndex = this.AllMechanics.indexOf(item)
    const index = this.AllMechanics.indexOf(item)
    if (confirm('Are you sure you want to delete this item?')) {
      // this.loader = true
      axios.delete(`/users/${this.delItem.id}`)
      .then((response) => {
        this.AllMechanics.splice(index, 1)
        // this.loader = false
...
mounted() {
  this.loader=true
  axios.post('getMechanics')
  .then((response) => {
    this.AllMechanics = response.data
  })
  .catch((error) => {
    this.errors = error.response.data.errors
  })
